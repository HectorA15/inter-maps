...
try {
    (maplibregl as unknown as MapLibreWithProtocol).addProtocol('pmtiles', protocol.tile);
} catch (e: unknown) {
...
const estiloClaro: StyleSpecification = {
    ...
    layers: [
        ...
        },
        {
            ...
            paint: {
                'line-color': '#adb5bd',
                'line-width': ['interpolate', ['linear'], ['zoom'], 13, 1.2, 15, 2.5, 19, 6],
            },
        },
        // Nombres solo de calles principales (major_road/arterial) para no saturar
        {
            ...
            source: 'mapa-local',
            'source-layer': 'roads',
            filter: ['in', ['get', 'kind'], ['literal', ['major_road', 'arterial', 'highway', 'trunk']]],
            layout: {
                'text-field': ['get', 'name'],
        ...
        },
        {
            ...
            type: 'line',
            source: 'campus-limite',
            paint: { 'line-color': '#a7d8a7', 'line-width': 1.5, 'line-dasharray': [4, 2] },
        },
        ...
        // Deportes (canchas) - verde tenue derivado de paleta, filtrado por tipo_edificio
        {
            ...
            paint: { 'fill-color': '#d3f9d8', 'fill-outline-color': '#69db7c', 'fill-opacity': 0.9 },
        },
    ],
};
...
export function MapaCampus() {
    ...
    const [bounds, setBounds] = useState<[number, number, number, number]>(FALLBACK_BOUNDS);
    const [seleccionado, setSeleccionado] = useState<{ fid: number; nombre: string } | null>(null);
    ...
    useEffect(() => {
        ...
        p.getHeader()
            ...
            .catch((err: unknown) => {
                console.warn('[MapaCampus] no se pudo leer header pmtiles, usando fallback:', err);
            });
        return () => {
            cancelado = true;
        };
    ...
    const handleMove = useCallback((evt: ViewStateChangeEvent) => setViewState(evt.viewState), []);

    const handleClick = useCallback((e: ViewStateChangeEvent['target'] extends never ? never : unknown) => {
        // react-map-gl pasa MapLayerMouseEvent con features
        const event = e as unknown as { features?: Array<{ properties: Record<string, unknown> }> };
        const feature = event.features?.[0];
        if (feature?.properties) {
            const props = feature.properties as { fid?: number; nombre_zona?: string; nombre?: string };
    ...
    return (
        <div className="mapa-campus-container">
            ...
            {seleccionado && (
                ...
                >
                    <div>
                        <div style={{ fontWeight: 600, color: '#212529', fontSize: '14px' }}>{seleccionado.nombre}</div>
                        <div style={{ color: '#868e96', fontSize: '12px' }}>ID: {seleccionado.fid}</div>
                    </div>
                    <button
