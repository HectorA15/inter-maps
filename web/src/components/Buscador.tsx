...
export function Buscador({ datos, onSelect, placeholder = "Buscar lugar, aula o edificio..." }: BuscadorProps) {
    ...
    useEffect(() => {
        ...
        if (datos && datos.length > 0) {
            ...
            const filtrados = datos.filter((item) => {
                const nombreNorm = item.nombre.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase();
    ...
    const manejarKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if (e.key === 'ArrowDown') {
            e.preventDefault();
            setIndiceSeleccionado((prev) => (prev < resultados.length - 1 ? prev + 1 : 0));
        } else if (e.key === 'ArrowUp') {
            e.preventDefault();
            setIndiceSeleccionado((prev) => (prev > 0 ? prev - 1 : resultados.length - 1));
        } else if (e.key === 'Enter') {
            e.preventDefault();
    ...
    return (
        <div className="contenedor-buscador-relativo" ref={contenedorRef}>
            <div className="input-wrapper">
                <svg className="icono-buscar" aria-hidden="true" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                {!cargando && textoBusqueda && (
                    <button className="boton-limpiar" onClick={limpiarBusqueda} title="Limpiar" aria-label="Limpiar busqueda">
                ...
                <ul className="dropdown-resultados">
                    {resultados.length > 0 ? (
                        resultados.map((item, idx) => (
                            ...
                            >
                                <span className={`badge-tipo ${item.tipo === 'EDIFICIO' ? 'badge-edificio' : 'badge-espacio'}`}>
                                    {item.tipo === 'EDIFICIO' ? 'Edificio' : 'Lugar'}
                                </span>
                                <span className="nombre-resultado">{capitalizarNombre(item.nombre)}</span>
                            </li>
                        ))
                    ) : (
                        <li className="item-sin-resultados">Sin resultados para "{textoBusqueda}"</li>
                    )}
                </ul>
            )}
        </div>
